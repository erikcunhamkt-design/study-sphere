...
export const CALLOUT_TYPES = ["info", "attention", "success"];
...
export function isSafeUrl(value) {
    if (value.trim() === "")
        return true; // vazio (em edição/upload) é inofensivo
...
const bookmarkPropsSchema = z
    ...
    .partial();
function propsSchemaForType(type) {
    switch (type) {
        case "heading":
...
export const lessonBlockSchema = z.lazy(() => z
    .object({
    id: z.string().min(1),
    ...
    const isTableContent = !!block.content && !Array.isArray(block.content) && block.content.type === "tableContent";
...
function walkTree(blocks, depth, stats) {
...
export const lessonDocumentSchema = z.array(lessonBlockSchema).superRefine((blocks, ctx) => {
    const stats = { count: 0, maxDepth: blocks.length > 0 ? 1 : 0, ids: [] };
