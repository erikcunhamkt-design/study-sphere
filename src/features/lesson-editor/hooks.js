...
export function lessonDocumentKey(userId, lessonId) {
    return ["lesson-document", userId, lessonId];
}
export function lessonDocumentVersionsKey(userId, documentId) {
    return ["lesson-document-versions", userId, documentId];
}
export function useLessonDocument(lessonId) {
    const { user } = useAuth();
    return useQuery({
...
export function useLessonDocumentVersions(documentId) {
    const { user } = useAuth();
    return useQuery({
 ...
 */
export function useSaveLessonDocument(lessonId) {
    const { user } = useAuth();
    const qc = useQueryClient();
    return useMutation({
        mutationFn: ({ content, schemaVersion, expectedVersion, }) => api.saveLessonDocument(lessonId, content, schemaVersion, expectedVersion),
        onSuccess: (result) => {
...
export function useCheckpointLessonDocument(lessonId) {
    const { user } = useAuth();
    const qc = useQueryClient();
    return useMutation({
...
export function useRestoreLessonDocumentVersion(lessonId) {
    const { user } = useAuth();
    const qc = useQueryClient();
    return useMutation({
        mutationFn: (version) => api.restoreLessonDocumentVersion(lessonId, version),
...
export function usePublishLessonDocument(lessonId) {
    const { user } = useAuth();
    const qc = useQueryClient();
    return useMutation({
