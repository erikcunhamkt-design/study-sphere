...
import { deleteDraft, saveDraft } from "./drafts-db";
 ...
 */
export function useLessonAutosave({ lessonId, initialVersion, initialSchemaVersion, }) {
    const { user } = useAuth();
    ...
    const attemptSave = useCallback(async (content, schemaVersion, expectedVersionForSave) => {
        setStatus("saving");
        try {
    ...
    const performSave = useCallback(async (content, schemaVersion, expectedVersionForSave) => {
        if (!user)
            return;
    ...
    const scheduleSave = useCallback((content, schemaVersion) => {
        setStatus("editing");
        if (debounceTimer.current)
            clearTimeout(debounceTimer.current);
    ...
    const resolveConflictKeepMine = useCallback(async (newExpectedVersion) => {
        if (!conflict)
            return;
        ...
        await performSave(conflict.attemptedContent, conflict.attemptedSchemaVersion, newExpectedVersion);
