...
const FLASHCARD_COLUMNS = "id, user_id, lesson_id, deck_id, source_block_id, front, back, state, learning_step, interval_days, ease, reps, lapses, due_at, is_archived, created_at, updated_at";
...
export async function fetchFlashcardsByDeck(userId, deckId) {
    const { data, error } = await supabase
        .from("flashcards")
        .select(FLASHCARD_COLUMNS)
        .eq("user_id", userId)
...
export async function fetchDueFlashcardsByDeck(userId, deckId) {
    const nowIso = new Date().toISOString();
    const { data, error } = await supabase
        .from("flashcards")
        .select(FLASHCARD_COLUMNS)
...
export async function updateFlashcardContent(flashcardId, input) {
    const { data, error } = await supabase
        .from("flashcards")
        .update({
        lesson_id: input.lessonId,
...
export async function setFlashcardArchived(flashcardId, isArchived) {
...
export async function setFlashcardsDeck(flashcardIds, deckId) {
    const { error } = await supabase
        .from("flashcards")
 ...
 */
export async function submitFlashcardReview(flashcardId, rating) {
...
export async function fetchFlashcardReviews(userId, sinceIso) {
    const { data, error } = await supabase
        .from("flashcard_reviews")
        .select("id, user_id, flashcard_id, reviewed_at, rating, resulting_state, resulting_interval_days, resulting_ease")
        .eq("user_id", userId)
