...
import { flashcardFormSchema, resolveFrontContentForSubmit, textFromInlineContent, plainTextToInlineContent, } from "./schema";
...
export function FlashcardFormDialog({ open, onOpenChange, prefill, flashcard, }) {
    ...
    useEffect(() => {
        ...
        if (flashcard) {
            ...
            setDeckId(null);
            setOriginalFront(prefill?.frontContent ? { text: flatFront, content: prefill.frontContent } : null);
        }
        setError(null);
    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            toast.error(isEditing ? "Não foi possível atualizar o cartão" : "Não foi possível criar o cartão");
        }
        finally {
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <Textarea id={frontId} ref={frontRef} value={front} maxLength={2000} rows={3} placeholder="Ex.: O que é RLS?" aria-invalid={!!error} onChange={(e) => setFront(e.target.value)}/>
          </div>

          <div className="space-y-2">
            <Label htmlFor={backId}>Verso</Label>
            <Textarea id={backId} value={back} maxLength={2000} rows={3} placeholder="Ex.: Row Level Security — regra de acesso avaliada pelo banco em cada linha." aria-invalid={!!error} onChange={(e) => setBack(e.target.value)}/>
          </div>
          ...
          <div className="space-y-2">
            ...
            <Select value={deckId ?? AVULSO_VALUE} onValueChange={(v) => setDeckId(v === AVULSO_VALUE ? null : v)}>
              <SelectTrigger id="flashcard-deck">
                <SelectValue placeholder="Nenhum"/>
              </SelectTrigger>
              <SelectContent>
                <SelectItem value={AVULSO_VALUE}>Nenhum</SelectItem>
                {activeDecks.map((d) => (<SelectItem key={d.id} value={d.id}>
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 rounded-full" style={{ backgroundColor: d.color ?? "#3b82f6" }}/>
                      {d.name}
                    </div>
                  </SelectItem>))}
              </SelectContent>
          ...
          <DialogFooter>
            <Button type="submit" disabled={isPending}>
              {isPending ? (<Loader2 className="h-4 w-4 animate-spin" aria-hidden/>) : isEditing ? ("Salvar alterações") : ("Criar cartão")}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
