...
export function RecordacaoAtivaHub({ onBack }) {
    ...
    return (<div className="space-y-4 rounded-xl border border-border bg-surface p-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <button onClick={() => setView("flashcards")} disabled={loadingFlashcards || !dueFlashcards?.length} className="flex items-center gap-3 rounded-lg border border-border bg-background px-4 py-3 transition-colors hover:border-primary/40 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        <button onClick={() => setView("questions_list")} className="flex items-center gap-3 rounded-lg border border-border bg-background px-4 py-3 transition-colors hover:border-primary/40">
          <span className="grid h-9 w-9 place-items-center rounded-lg bg-primary/10 text-primary">
      ...
      <button type="button" onClick={onBack} className="text-sm text-muted-foreground underline-offset-2 hover:underline">
        Voltar
      </button>
    </div>);
}
