 ...
 * disponível para essa escrita. Mesma classe de desvio de relógio do achado
 * do timer da Fase 05.1, mas sem efeito visual aqui (só grava, não exibe
 * contagem regressiva contra esse valor).
 */
export async function finishStudySession(
  ...
  const { data, error } = await supabase
    .from("study_sessions")
    .update({ ended_at: new Date().toISOString(), details: details as unknown as Json })
    .eq("id", sessionId)
    .select(STUDY_SESSION_COLUMNS)
    .single();
  if (error) throw error;
