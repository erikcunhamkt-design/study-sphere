...
import { validateQuestionForm } from "./schema";
import { useCreateQuestion, useUpdateQuestion } from "./hooks";
const AVULSO_VALUE = "__avulso__";
const MIN_OPTIONS = 2;
const MAX_OPTIONS = 6;
export function QuestionFormDialog({ open, onOpenChange, question, prefill }) {
    ...
    async function handleSubmit(e) {
        e.preventDefault();
        if (createQuestion.isPending || updateQuestion.isPending)
            return;
        const formValue = type === "multipla_escolha"
            ? {
                type,
                lessonId,
                statement,
        ...
        setError(null);
        const input = parsed.data.type === "multipla_escolha"
            ? {
        ...
        try {
            ...
            toast.error(isEditing ? "Não foi possível atualizar a questão" : "Não foi possível criar a questão");
        }
    }
    return (<Dialog open={open} onOpenChange={(next) => !isPending && onOpenChange(next)}>
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={(e) => void handleSubmit(e)} className="space-y-4">
          <div className="space-y-2">
            <Label>Tipo</Label>
            <RadioGroup value={type} onValueChange={(v) => setType(v)} className="grid grid-cols-2 gap-2">
          ...
          <div className="space-y-2">
            ...
            <Textarea id={statementId} value={statement} maxLength={5000} rows={3} placeholder="Ex.: O que é RLS?" onChange={(e) => setStatement(e.target.value)}/>
          </div>

          {type === "multipla_escolha" ? (<div className="space-y-2">
              <Label>Alternativas</Label>
              <RadioGroup value={correctOptionIndex === null ? undefined : String(correctOptionIndex)} onValueChange={(v) => setCorrectOptionIndex(Number(v))} className="space-y-2">
                {options.map((opt, index) => (<div key={index} className="flex items-center gap-2">
                    <RadioGroupItem value={String(index)} aria-label={`Marcar alternativa ${index + 1} como correta`}/>
                    <Input value={opt} maxLength={500} placeholder={`Alternativa ${index + 1}`} onChange={(e) => updateOption(index, e.target.value)}/>
                    <Button type="button" variant="ghost" size="icon" aria-label="Remover alternativa" disabled={options.length <= MIN_OPTIONS} onClick={() => removeOption(index)}>
              ...
              <Textarea id="question-expected-answer" value={expectedAnswer} maxLength={5000} rows={3} placeholder="O que conta como acerto para esta questão" onChange={(e) => setExpectedAnswer(e.target.value)}/>
              <p className="text-xs text-muted-foreground">
                Sem correção automática: ao praticar, você mesmo diz se acertou.
              </p>
            </div>)}

          {error ? (<p className="text-xs text-destructive" role="alert">
              {error}
            </p>) : null}

          <div className="space-y-2">
            <Label htmlFor="question-lesson">Aula (opcional)</Label>
            <Select value={lessonId ?? AVULSO_VALUE} onValueChange={(v) => setLessonId(v === AVULSO_VALUE ? null : v)}>
              ...
              <SelectContent>
                ...
                {activeLessons.map((l) => (<SelectItem key={l.id} value={l.id}>
                    {l.title}
                  </SelectItem>))}
              </SelectContent>
            </Select>
          ...
          <DialogFooter>
            <Button type="submit" disabled={isPending}>
              {isPending ? (<Loader2 className="h-4 w-4 animate-spin" aria-hidden/>) : isEditing ? ("Salvar alterações") : ("Criar questão")}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
